...
# Additional permissions for ECS Task Execution Role
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:logs:${var.aws_region}:${local.account_id}:log-group:/aws/ecs/${var.project_name}*"
      }
    ]
  })
}
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "token.actions.githubusercontent.com:sub" = "repo:*/${var.project_name}:*"  # Allow any GitHub user
          }
        }
      }
    ]
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          for service in keys(local.services) : aws_ecr_repository.services[service].arn
        ]
      },
      {
        Effect = "Allow"
...
# Create additional secrets for JWT and other application secrets
