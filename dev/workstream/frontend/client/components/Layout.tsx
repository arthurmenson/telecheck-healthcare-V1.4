    ...
    return [
      {
        ...
        items: [
          ...
          { name: "Algorithm Config", href: "/algorithm-config", icon: Dna },
        ],
      },
      {
        id: "clinical",
      ...
      },
      {
        ...
        items: [
          ...
          { name: "System Settings", href: "/admin/system-settings", icon: Dna },
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          {navigation.map((item: any) => {
            ...
            if (item.isGroup) {
              ...
              const isExpanded = expandedGroups.includes(item.id || item.name.toLowerCase().replace(' ', '-'));
              ...
              return (
                <div key={item.name} className="space-y-1">
                  {/* Group Header */}
                  <button
                    onClick={() => toggleGroup(item.id || item.name.toLowerCase().replace(' ', '-'))}
                    ...
                    {user?.role === "admin"
                      ? (() => {
                          ...
                          for (const group of navigation) {
                            if ((group as any).isGroup && (group as any).items) {
                              const foundItem = (group as any).items.find(
