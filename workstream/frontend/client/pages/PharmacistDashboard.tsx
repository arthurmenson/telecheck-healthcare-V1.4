...
export function PharmacistDashboard() {
  ...
  const pendingPrescriptions = prescriptions.filter(p => p.status === 'active' || p.status === 'filled');
  ...
  // Filter filled prescriptions for recent dispensing
  const recentDispensing = prescriptions.filter(p => p.status === 'filled').slice(0, 5);
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">
                              Patient: {item.patientId} • Qty: {item.quantity}
                            </div>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-sm text-muted-foreground">
                            {item.filledAt ? new Date(item.filledAt).toLocaleString() : 'Recently'}
                          </div>
                          <Badge
                            variant="outline"
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-bold text-foreground">
                      {metricsLoading ? (
                        ...
                        `${dashboardMetrics?.activePatients || 0}/${dashboardMetrics?.totalPatients || 0}`
                      )}
                    </span>
                  </div>
                  <Progress
                    value={dashboardMetrics?.totalPatients ?
                      (dashboardMetrics.activePatients / dashboardMetrics.totalPatients) * 100 : 0
                    }
                    className="h-2"
                  />
