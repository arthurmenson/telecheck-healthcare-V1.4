...
export function AdminDashboard() {
  ...
  const { data: systemActivities, isLoading: activitiesLoading } = useSystemActivities({ limit: 10 });
  const { data: securityAlerts, isLoading: alertsLoading } = useSecurityAlerts({ limit: 5 });
  const { data: platformStats, isLoading: statsLoading } = usePlatformStats();
  const { data: systemHealth, isLoading: healthLoading } = useSystemHealth();

  // Fallback data while loading or on error
  const usersByRole = systemMetrics?.usersByRole || [];
  const recentActivities = systemActivities || [];
  const securityAlertsData = securityAlerts || [];
  const platformStatsData = platformStats || [];

  // Default system metrics to prevent undefined access
  ...
  const isLoading = metricsLoading || activitiesLoading || alertsLoading || statsLoading || healthLoading;
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-foreground">
                    {safeSystemMetrics.totalUsers.toLocaleString()}
                  </p>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-foreground">
                    {safeSystemMetrics.systemUptime}%
                  </p>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-foreground">
                    {safeSystemMetrics.dailyTransactions.toLocaleString()}
                  </p>
